...
export default function Post() {
  ...
  // Load existing item if in edit mode. Use the id value (not the whole params
  // object) in the dependency array to avoid re-running the effect when the
  // params object identity changes on each render which would clobber user
  // edits while typing.
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Build payload without undefined to avoid clearing fields unintentionally
      ...
      // Always include status on edit (send exactly what's selected)
      if (isEdit) basePayload.status = status;

      if (isEdit) {
        console.debug("status before submit", status);
        console.debug("PUT /api/items payload", { id: params!.id, ...basePayload });
        const res = await apiRequest("PUT", `/api/items/${params!.id}`, basePayload);
        let updated: any = null;
        try {
          updated = await res.clone().json();
          console.debug("PUT /api/items response", updated);
        } catch {}
        // Optional sanity check
        if (updated && updated.status && status && updated.status !== status) {
          console.warn("Server returned a different status than selected", { selected: status, server: updated.status });
        }
        // Ensure lists show fresh data after edit
        await queryClient.invalidateQueries({ queryKey: ["/api/items"] });
        await queryClient.invalidateQueries({ queryKey: ["/api/user/items"] });
        // also refresh public browse list aggressively
        await queryClient.invalidateQueries({ queryKey: ["/api/items"] });
        navigate(`/dashboard`);
      } else {
        const res = await apiRequest("POST", "/api/items", { ...basePayload, status: "lost" });
        await res.json();
        // Ensure browse list refreshes immediately after post
        await queryClient.invalidateQueries({ queryKey: ["/api/items"] });
        navigate(`/browse`);
      }
    } catch (err: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 md:p-6">
      <h1 className="text-2xl font-bold mb-4">{isEdit ? (status === 'found' ? 'Edit Found Item' : 'Edit Lost Item') : 'Post Lost Item'}</h1>
      <form className="space-y-4" onSubmit={onSubmit} noValidate>
        <Input placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} required />

        <div className="space-y-2">
          <label className="text-sm font-medium">Photo <span className="text-xs text-muted-foreground">(optional)</span></label>
          <div
        ...
        {/* Status selection removed in edit mode; UI adapts based on existing status */}

        <Textarea placeholder="Description" value={description} onChange={(e) => setDescription(e.target.value)} required />

        <div className="flex gap-2 items-center">
          <Input placeholder={isEdit ? (status === 'found' ? 'Where was it found?' : 'Where was it lost?') : 'Location'} value={location} onChange={(e) => setLoc(e.target.value)} required />
        ...
        <Button type="submit" disabled={loading}>{loading ? (isEdit ? "Saving..." : "Posting...") : (isEdit ? (status === 'found' ? 'Update Found Item' : 'Update Lost Item') : 'Post Item')}</Button>
      </form>
    </div>
  );
}
